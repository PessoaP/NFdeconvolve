...
Ncomp = 5
#diralpha=((.75)**torch.arange(Ncomp)).to(device)
#dir_prior = torch.distributions.Dirichlet((diralpha/diralpha.sum())*250)

diralpha = torch.ones(Ncomp).to(device)#/Ncomp
dir_prior = torch.distributions.Dirichlet(torch.ones(Ncomp).to(device))
...
def prop_rhos(rhos,diralpha=diralpha,tol=1e-2):
    dir = torch.distributions.Dirichlet(rhos/tol)
    return dir.sample()

def log_propdist_rhos(prop,origin,diralpha=diralpha,tol=1e-2):
...
def update(mus,sigs,rhos,lp,tol=1e-2,tol_rho =1e-4,burnin=False):
...
for i in range(10000):
    mus,sigs,rhos,lp = update(mus,sigs,rhos,lp,tol=1e-2)
...
ax[1].set_xlabel(r'$x$')
plt.savefig('graphs/Bayes_WM_N_{}_{}_G_{}_{}_datapoints{}.png'.format(mu_a,sig_a,shape,scale,N),dpi=600)

# %%
df = pd.DataFrame(torch.vstack(thetas).cpu().numpy(),columns=['mu_1','mu_2','mu_3','mu_4','mu_5',
                                                              'sigma_1','sigma_2','sigma_3','sigma_4','sigmma_5',
                                                              'weight_1','weight_2','weight_3','weight_4','weight_5'])
df['log_post'] = torch.stack(lps).cpu().numpy()
df.to_csv('models/sum_wmbayes_'+filename.split('.')[0]+'datapoints_{}.csv'.format(N))

# %%
with open('report.csv', 'a') as file:
    file.write(filename+',wmbayes,'+str(N)+','+str(KL(p_gt,p,xb).item())+','+str(KL(p_gt,p_map,xb).item())+'\n')
